...
def model_eval(hist):
    ...
    for index, filename in enumerate(selected_images):
        # Load and process each image
        img_path = os.path.join(test_dir, filename)
        img_array = load_and_preprocess_image(img_path)  # Make sure this matches your preprocessing
        img = load_img(img_path, target_size=image_size)
        # Make predictions
        output = saved_model.predict(img_array)
        prediction = "Fresh" if output[0][0] > output[0][1] else "Rotten"
        ...
        x = index % grid_size[0] * image_size[0]
        y = index // grid_size[0] * image_size[1]
        grid_image.paste(overlay_img, (x, y))
    
    grid_image.save(os.path.join(save_dir, 'GRADCAMtest_predictions_grid.jpg'))
